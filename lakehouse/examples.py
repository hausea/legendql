...
from lakehouse import Ingest, DataProduct, AccessPoint, Legend, CSV, Scheduled, any, Pandas, \
    Polars, Lakehouse
...
from model.schema import Dataset, enterprise, producer_entitled
...
mat_view = MaterializedView(
    source={"mv_metadir": Legend(metadir)},
    versioning = batch_milestone_full,
    classification=enterprise,
    trigger=any([raw_meta, raw_corp]) # could be all() as well
)

lh.run_materialized_view(mat_view)
...
pos = Ingest(
    source=CSV(file="/user/test/positions_2025.csv", column_delimiter="|"),
    versioning=batch_milestone_incremental,
...
consumer_view = MaterializedView(
    source={"rle_positions": Legend(rle)},
    versioning=batch_milestone_full,
    classification=enterprise,
    trigger=Scheduled()
)

lh.run_materialized_view(consumer_view)
